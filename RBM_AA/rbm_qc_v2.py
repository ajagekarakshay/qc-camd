...
class QRBM():
    ...
    def qsample(self, num_reads=10000):
        samples = []
        energies = []
        num_occur = []
    ...
    def weight_update(self, visible_data, lr=0.1, alpha=1):
        ...
        self.W = alpha * self.W + lr * (np.dot(visible_data.T, ph_sample) / batch_samples - vh_mean)
        self.b = alpha * self.b + lr * (np.mean(visible_data, axis=0) - v_mean)
